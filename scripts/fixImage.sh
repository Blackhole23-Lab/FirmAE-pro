...
if (${FIRMAE_BOOT}); then
  ...
  # fix broken symlinks pointing to /dev/null (common in OpenWrt firmware)
  for DIR in /bin /sbin /usr/bin /usr/sbin; do
    if [ -d "$DIR" ]; then
      for FILE in `${BUSYBOX} find "$DIR" -maxdepth 1 -type l`; do
        TARGET=$(${BUSYBOX} readlink "$FILE")
        if [ "$TARGET" = "/dev/null" ]; then
          ${BUSYBOX} rm -f "$FILE"
          ${BUSYBOX} ln -s /bin/busybox "$FILE"
        fi
      done
    fi
  done
