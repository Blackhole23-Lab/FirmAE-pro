...
# Read detected firmware kernel version if available
FW_KERNEL_VER=""
if [ -f "${WORK_DIR}/kernel_version" ]; then
    FW_KERNEL_VER=$(cat ${WORK_DIR}/kernel_version)
    if [ "${FW_KERNEL_VER}" = "unknown" ]; then
        FW_KERNEL_VER=""
    fi
fi

if [ -n "${FW_KERNEL_VER}" ]; then
    KERNEL=`get_kernel ${ARCHEND} ${FW_KERNEL_VER}`
    echo "[*] Using version-matched kernel for ${ARCHEND} (firmware: ${FW_KERNEL_VER})"
else
    KERNEL=`get_kernel ${ARCHEND}`
    echo "[*] Using default kernel for ${ARCHEND}"
fi
...
%(QEMU_ENV_VARS)s ${QEMU} ${QEMU_BOOT} %(QEMU_CPU)s -m 1024 -M ${QEMU_MACHINE} -kernel ${KERNEL} \\
    %(QEMU_DISK)s -append "root=${QEMU_ROOTFS} %(QEMU_CONSOLE)s nandsim.parts=64,64,64,64,64,64,64,64,64,64 %(QEMU_INIT)s rw debug ignore_loglevel print-fatal-signals=1 FIRMAE_NET=${FIRMAE_NET} FIRMAE_NVRAM=${FIRMAE_NVRAM} FIRMAE_KERNEL=${FIRMAE_KERNEL} FIRMAE_ETC=${FIRMAE_ETC} ${QEMU_DEBUG}" \\
...
def findMacChanges(data, endianness):
    ...
    if endianness == "eb":
        fmt = ">I"
    elif endianness == "el" or endianness == "hf":
        fmt = "<I"
...
def qemuNetworkConfig(arch, network, isUserNetwork, ports):
    ...
    assigned = []
    interfaceNum = 4
    if arch in ["arm", "arm64"] and checkVariable("FIRMAE_NET"):
        interfaceNum = 1
...
def qemuCmd(iid, network, ports, network_type, arch, endianness, qemuInitValue, isUserNetwork):
    ...
    if arch == "mips":
        ...
        qemuConsole = "console=ttyS0"
        qemuCpu = ""
        if endianness == "el" or endianness == "hf":
        ...
        qemuDisk = "-drive if=none,file=${IMAGE},format=raw,id=rootfs -device virtio-blk-device,drive=rootfs"
        qemuConsole = "console=ttyS0 earlycon"
        qemuCpu = "-cpu cortex-a57"
        if endianness == "el":
            qemuEnvVars = "QEMU_AUDIO_DRV=none"
        else:
            raise Exception("aarch64 only supports little-endian")
    else:
...
def inferNetwork(iid, arch, endianness, init):
    ...
    cmd += "{0}/run.{1}.sh \"{2}\" \"{3}\" ".format(SCRIPTDIR,
                                                    archEndToFirmAE(arch, endianness),
                                                    iid,
                                                    qemuInitValue)
    cmd += " 2>&1 > /dev/null"
    os.system(cmd)
...
def process(iid, arch, endianness, makeQemuCmd=False, outfile=None):
    ...
    for init in open(SCRATCHDIR + "/" + str(iid) + "/init").read().split('\n')[:-1]:
        ...
        if filterNetworkList:
            ...
            os.system('./scripts/check_emulation.sh {} {}'.format(iid, archEndToFirmAE(arch, endianness)))
...
def archEnd(value):
    ...
    if tmp.startswith("mips"):
        ...
        return (arch, end)
    elif tmp == "aarch64" or tmp == "arm64el":
        arch = "arm64"
    ...
    if arch == "arm64" and end is None:
        end = "el"
    return (arch, end)

def archEndToFirmAE(arch, endianness):
    """Convert internal arch+endianness to FirmAE architecture name."""
    if arch == "arm64":
        return "aarch64"
    if arch == "arm" and endianness == "hf":
        return "armhf"
