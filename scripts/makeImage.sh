...
if (! ${FIRMAE_ETC}); then
  sed -i 's/sleep 15/sleep 5/g' "${IMAGE_DIR}/firmadyne/network.sh"
  sed -i 's/sleep 30/sleep 10/g' "${IMAGE_DIR}/firmadyne/run_service.sh"
...
sync
umount "${IMAGE_DIR}" || umount -l "${IMAGE_DIR}" || true
