...
if check_number $1; then
    echo "Usage: run.aarch64.sh <image ID>"
    exit 1
fi
IID=${1}
QEMU_INIT=${2}
...
IMAGE=`get_fs ${IID}`
KERNEL=`get_kernel "aarch64" false`
QEMU_MACHINE=`get_qemu_machine "aarch64"`
QEMU_ROOTFS=`get_qemu_disk "aarch64"`

if (${FIRMAE_NET}); then
  QEMU_NETWORK="-device virtio-net-device,netdev=net0 -netdev user,id=net0,hostfwd=tcp::8080-:80,hostfwd=tcp::8443-:443"
else
