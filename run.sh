...
echo "[*] FirmAE started at: $(date '+%Y-%m-%d %H:%M:%S')"
...
function run_emulation()
{
    ...
    # First, check if inferKernel.py already extracted version from kernel binary
    if [ -f "${WORK_DIR}/kernelVersion" ] && [ -s "${WORK_DIR}/kernelVersion" ]; then
        KERNEL_VERSION=$(head -1 "${WORK_DIR}/kernelVersion" | grep -oP 'Linux version \K[0-9]+\.[0-9]+[^\s]*' | head -1)
    fi

    # If not found, try to extract from /lib/modules/ in tarball
    if [ -z "${KERNEL_VERSION}" ]; then
        KERNEL_VERSION=$(tar -tzf ./images/$IID.tar.gz 2>/dev/null | grep -oP 'lib/modules/\K[0-9]+\.[0-9]+[^\s/]*' | head -1)
    fi
    ...
    echo -e "[\033[32m+\033[0m] Firmware kernel version: ${KERNEL_VERSION}"
    echo "${KERNEL_VERSION}" > "${WORK_DIR}/kernel_version"
