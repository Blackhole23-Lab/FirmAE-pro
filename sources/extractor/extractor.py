...
class Extractor(object):
    ...
    @staticmethod
    def io_find_rootfs(start, recurse=True, max_depth=10):
        """
        Attempts to find a Linux root directory.
        Binwalk 3 compatibility: Deep recursive search for nested filesystems.
        """
        ...
        # Binwalk 3: Deep recursive search (not just one level)
        # Multiple filesystems may be extracted in nested directories
...
class ExtractionItem(object):
    ...
    def extract(self):
        ...
        try:
            ...
            # Binwalk 3: Use subprocess to call binwalk CLI
            # -M: matryoshka (recursive extraction)
            # -e: extract known file types
            # -d: output directory
            cmd = ["binwalk", "-M", "-e", "-d", self.temp, self.item]
            self.printf(f">> Running: {' '.join(cmd)}")
            result = subprocess.run(cmd, capture_output=True, text=True)

            # Binwalk 3 extracts to: <output_dir>/<basename>.extracted/
            basename = os.path.basename(self.item)
            dir_name = os.path.join(self.temp, f"{basename}.extracted")

            # Check if extraction directory exists and search for filesystem
            if os.path.exists(dir_name):
                self.printf(f">> Extraction directory: {dir_name}")

                # Search for Linux root filesystem in extracted files
                if not self.get_rootfs_status():
                    unix = Extractor.io_find_rootfs(dir_name)
                    if unix[0]:
                        self.printf(f">>>> Found Linux filesystem in {unix[1]}!")
                        if self.output:
                        ...
                        if self.update_status():
                            self.printf(">> Extraction completed!")
                            return True

            # Also scan for signature information (for kernel detection, etc.)
            for module in binwalk.scan(self.item, signature=True, quiet=True):
                prev_entry = None
...
#        print ('-=----------------------------')
    ...
    def _check_firmware(self, module, entry, dir_name=None):
        """
    ...
    def _check_kernel(self, module, entry, dir_name=None):
        """
    ...
    def _check_rootfs(self, module, entry, dir_name=None):
        """
        ...
        if 'filesystem' in desc or 'archive' in desc or 'compressed' in desc:
            if self.get_rootfs_status(): return True
                ...
                if dir_name:
                    self.printf(f">>>> Checking for rootfs in: {dir_name}")
                    self.printf(f">>>> Directory exists: {os.path.exists(dir_name)}")
                    if os.path.exists(dir_name):
                        self.printf(f">>>> Directory contents: {os.listdir(dir_name)[:5]}")
                    unix = Extractor.io_find_rootfs(dir_name)
                    if not unix[0]:
    ...
    def _check_recursive(self, module, entry, dir_name=None):
        """
